...
class FlexDataset(Dataset):
    def __init__(self, dataset_name=None, root=None, device=None):
        ...
        # 检查数据集的名称是否正确
        assert dataset_name in ['tox21', 'bace', 'bbbp',
                                'clintox', 'esol', 'freesolv', 'lipo', 'sider',]
        # 检查根路径是否给出
    ...
    def load_raw_dataset(self):
        ...
        if self.dataset_name == 'tox21':
            ...
            if self.has_downloaded():
                dataset = BACE(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                               edge_featurizer=BOND_FEATURIZER), load=True, cache_file_path=save_path)
            else:
                dataset = BACE(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                               edge_featurizer=BOND_FEATURIZER), load=False, cache_file_path=save_path)
            ...
            if self.has_downloaded():
                dataset = BBBP(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                               edge_featurizer=BOND_FEATURIZER), load=True, cache_file_path=save_path)
            else:
                dataset = BBBP(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                               edge_featurizer=BOND_FEATURIZER), load=False, cache_file_path=save_path)
            ...
            if self.has_downloaded():
                dataset = ESOL(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                               edge_featurizer=BOND_FEATURIZER), load=True, cache_file_path=save_path)
            else:
                dataset = ESOL(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                               edge_featurizer=BOND_FEATURIZER), load=False, cache_file_path=save_path)
            ...
            if self.has_downloaded():
                dataset = FreeSolv(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                                   edge_featurizer=BOND_FEATURIZER), load=True, cache_file_path=save_path)
            else:
                dataset = FreeSolv(SMILESToBigraph(node_featurizer=ATOM_FEATURIZER,
                                   edge_featurizer=BOND_FEATURIZER), load=False, cache_file_path=save_path)
            ...
            if self.has_downloaded():
                dataset = Lipophilicity(SMILESToBigraph(
                    node_featurizer=ATOM_FEATURIZER, edge_featurizer=BOND_FEATURIZER), load=True, cache_file_path=save_path)
            else:
                dataset = Lipophilicity(SMILESToBigraph(
                    node_featurizer=ATOM_FEATURIZER, edge_featurizer=BOND_FEATURIZER), load=False, cache_file_path=save_path)
    ...
    def process(self, raw_dataset):
        ...
        # 处理分子图特征
        Graphs, smiles_list, label_list, ats_list = [], [], [], []
        for _, data in tqdm(enumerate(iter(raw_dataset))):
            smiles, g, label = data[0], None, data[2].to(self.device)
            try:
    ...
    def task_pos_weights(self):
        ...
        task_pos_weight[num_pos > 0] = (
            (len(self.labels) - num_pos) / num_pos)[num_pos > 0]
